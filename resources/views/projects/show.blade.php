      ...
      <div>
        ...
        <div class="mt-2 md:flex md:items-center md:justify-between">
          ...
          <div class="mt-4 flex-shrink-0 flex md:mt-0 md:ml-4">
            <a href="{{ route('projects.remove', $project) }}" class="ml-3 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
              {{ __('Delete') }}
            </a>
        ...
        <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
          <dl class="sm:divide-y sm:divide-gray-200">
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                @if( $project->link )
                  <a href="{{ $project->link }}" class="text-indigo-600 hover:text-indigo-900">{{ $project->link }}</a>
                @else
                  -
                @endif
              </dd>
            ...
            <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                @if( $project->published_at )
                  {{ $project->published_at->isoFormat('D MMMM YYYY') }}
                @else
                  -
                @endif
              </dd>
    ...
    <div class="max-w-7xl mx-auto">
      <div class="bg-white shadow overflow-hidden rounded-md">
        <ul class="divide-y divide-gray-200">
            ...
            <div class="-ml-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-nowrap">
              <div class="ml-4 mt-4">
                ...
                <p class="mt-1 text-sm text-gray-500">
                  {{ __('These blocks will provide the content for this project.') }}
                </p>
              </div>
              <div class="ml-4 mt-4 flex-shrink-0">
                <a href="{{ route('projects.blocks.create', $project) }}" class="relative inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  {{ __('Create new block') }}
                </a>
              </div>
            </div>
          ...
          @if( $project->page )
            @foreach( $project->page->blocks()->orderBy('order', 'asc')->get() as $block )
              <li>
