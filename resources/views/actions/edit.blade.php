      ...
      <form action="{{ $action->exists ? route('actions.update', $action) : route('blocks.actions.store', $action->block) }}" method="POST">
        ...
        <div class="space-y-6">
          ...
          <div class="flex justify-end">
            <a href="{{ $action->exists ? route('pages.show', $action) : route('pages.index') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
              {{ __('Cancel') }}
            </a>
